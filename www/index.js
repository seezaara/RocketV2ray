!function () {
    ...
    try {
        ...
        utils.init({
            ...
            post: userdata.socks5.split(":")[1],
            vpn: userdata.mode,
        })
    } catch (e) {
        var userdata = { "language": "fa", "dns": "1.1.1.1", "socks5": "127.0.0.1:58694", "handle": "0.0.0.0/0", "bypass": "10.0.0.0/8\n172.16.0.0/16\n192.168.0.0/16", "mode": "on", "site": "on" }
    ...
    function load() {
        ...
        for (const i in configs) {
            ...
            <div class="item-title fillf">
                <div class="title-name">${configs[i].ps}</div>
                <div class="title-ip">${configs[i].add}:${configs[i].port}</div>
            </div>
    ...
    $(".list").addEventListener("click", function (event) {
        ...
        if (event.target.closest(".item-share")) {
            const text = utils.config_share(ind)
